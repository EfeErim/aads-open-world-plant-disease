...
def test_continual_loss_rejects_out_of_range_labels(label: int) -> None:
    with pytest.raises(ValueError, match="class range"):
        continual_loss(
            logits=np.array([[1.0, 2.0]]),
            labels=np.array([label]),
            current_features=np.zeros((1, 2)),
            teacher_features=np.zeros((1, 2)),
            adapter_parameters=np.zeros(1),
        )


def test_continual_loss_rejects_non_integer_labels() -> None:
...
@pytest.mark.parametrize("surface", ["logits", "features", "adapter"])
def test_continual_loss_rejects_non_finite_inputs(surface: str) -> None:
    logits = np.array([[1.0, 2.0]])
    current_features = np.zeros((1, 2))
    adapter_parameters = np.zeros(1)
    if surface == "logits":
        logits[0, 0] = np.nan
    elif surface == "features":
        current_features[0, 0] = np.inf
    else:
        adapter_parameters[0] = np.nan
...
def test_loss_weights_must_be_finite_and_non_negative() -> None:
    with pytest.raises(ValueError, match="finite and non-negative"):
        LossWeights(distillation=-0.1)
